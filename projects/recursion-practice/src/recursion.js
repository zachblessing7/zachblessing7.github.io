...
var arraySum = function(array) {
  ...
  //recursive case
    ...
    return array[0][0] + sum(array.slice(1));
  }
...
var range = function(x, y) {
  if(x === y || (x + 1) === y || (y + 1) === x){
    ...
    var list = range(y, x - 1);
    list.push(x - 1);
    return list.reverse();
  } else if (y - x === 2) {
    return [x + 1];
  } else {
    var list2 = range(x, y - 1);
    list2.push(y - 1);
    return list2;
  }
};
...
var palindrome = function(string) {
  var lowerString = string.replace(/[^A-Z]/ig, "").toLowerCase();
  
    if (lowerString.length < 2){
        return true;
    } 
  ...
  // console.log(`this is words 2 ${words2}`);
  // var words3 = words2.reverse();
  // console.log(`this is words 3 ${words3}`);
  // if(string.toLowerCase().split(" ").join("") === words2){
  //   return true;
  // } else {
  //   return false;
  // }
...
var createArray = function(str){
  ...
  var list = createArray(str.slice(1));
  list.unshift(str[0]);
  return list;
};
